...
- **@32K: delta trains 1.27× faster per step (fwd 1.40×, bwd 1.24×),
  +27.5% tokens/sec, equal memory.** GPU-hours for the 500-step 32K run:
  0.85h vs 1.08h.
- **@8K: delta is ~4% SLOWER** (sparse window covers 37% of context;
  correction overhead exceeds the attention savings). The honest framing:
  the training speedup is a LONG-context effect, growing with length —
  same shape as every other result in this project. sems 0.2–7.8ms.
...
- **delta-trained − dense-trained: −0.014 ± 0.022 (n=200, 21W/24L/155T)**
  — indistinguishable, tie-dominated. delta-trained − base: −0.018 ±
  0.028; dense-trained − base: −0.004 ± 0.023. No arm damages tasks.
...
held-out chunks 20K docs deep, deterministic)
...
- Retained fraction of the dense-finetune benefit under dense eval:
  ...
  pattern replicates; the retention FRACTION is corpus-dependent (note
  absolute arXiv losses are ~2.4x smaller, so the denominators are tiny).
- Pipeline tax on arXiv is much smaller than PG19 (base: 0.155 loss vs
  ~0.6 ppl-scale on books) — cross-doc-packed papers have shorter
  long-range dependencies than 32K book chunks, consistent with the
  mechanism.

Triad status: T1 measured (27% @32K, nil @8K), T2 parity confirmed
(paired), T3 replicated (direction + paired significance on both evals).
Box self-terminated; total cost ~$20.
