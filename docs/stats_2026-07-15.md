...
probe on the box (`~/margin_probe.log`, rescued): the top-2 logit margins
at the three checked-prompt divergence points were **0.125 / 0.000 / 0.250**
(0–2 bf16 ulps — literal argmax coin flips; in 2/3 cases a fresh dense
forward sided with the SPEC output against the sequential reference). Gate
v4 (`ae6a30c`): early divergences get a one-forward margin check; margin
≤ 0.5 → recorded tie, run continues; larger → hard fail. The rerun cell
carries the annotation `parity 174+1tie(17@0.12)`.
...
- Delta pos-1 on GovReport: 0.9931–0.9974 everywhere (structural anchor
  accept). ce32k/delta/K=8: accepted-per-verify 6.18 → **τ ≈ 7.2**.
- **Fair-timing speedups (synced timers, prefill both sides, 3 checked
  prompts/config):** ce32k delta K=4 30.17 vs dense 24.81 (**1.22×**),
  K=2 1.21×; dftmix delta K=4 1.18×; base delta K=2 1.20×. Sparse K=8 is a
  slowdown (15.7–22.0). Small-n timing — direction trustworthy, second
  decimal not. Same-size draft, so the ceiling is the attention share of
  decode cost; these are consistent with that.
...
on GovReport) — the drafted positions covering the needle span are exactly
where it dies. Jeff's "impossible by construction" holds for the needle
content; the residual delta acceptance is the predictable answer-template
boilerplate.
