...
(+12.8 points: dft/sparse 0.760 vs base/sparse 0.632) — Jeff's "SWA tokens
learn to deal with the delta information" conjecture, in data; (3) the
...
probe confirmed the intervention: achieved anchor_grad_ratio ≈ 1.00 under
1/√γ (vs 3.41 unscaled). Eval run manually on the held box during the
...
Paired: gsqrt − gsinv = −0.0026 ± 0.0001 (gsqrt better); base − gsqrt =
+0.1286 ± 0.0032; cross-run pairing to delta_32k valid via the
deterministic chunks. Reading: damping the correction gradient hurts
