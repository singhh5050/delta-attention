...
## H. Benchmark transfer (bench32k box, runs `xm2j1gaa` v1 / `mtu4s1s9` enmc)

8 arms × LongBench QA + En.MC (per-sample logged). Aggregates show NO
consistent transfer of the perplexity edges (detach32k best on QA at .4705
yet worst on En.MC at .5328 — noise signature); nothing degraded either.
Base/8K arms replicate 07-13/14 values to ±0.003. Per-sample paired tests
possible from the archived _samples.csv.

## I. In flight / queued

- specdec2 (TRUE draft-and-verify: acceptance rate, accepted/verify,
  full-block rate, exact dense parity; GovReport + QA; base/ce32k/dft
  drafts) — first in the capacity queue
- bench32k box: En.MC 8-arm → dft+CE training → 8-arm 2×2 (ALL-DONE ~2:15pm)
- mmlu (10 arms ×1000, + directional-gradient probe rider), gradscale
  (1/√γ, 1/γ @32K), seedsdistill, seeds32k — queued behind capacity
