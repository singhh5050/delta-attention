...
- **Timing — CORRECTED (07-16 review).** The as-run 1.20–1.22× divided
  warm spec numerators by COLD dense baselines: refs are timed right after
  model load, and both quoted ce32k cells came from resume runs where
  ce32k was the first config (dense 24.81/24.83 cold vs **26.23 warm** for
  the identical config in re9piix8). Corrected against warm same-config
  baselines: **ce32k delta K=4 ≈ 1.15×, K=2 ≈ 1.15×, K=8 ≈ 0.97×
  (slowdown); dftmix delta K=4 = 1.18× (the one cell whose baseline was
  already warm); base delta K=2 ≈ 1.14× (estimate — base never ran warm).**
  Remaining known bias, unquantified: the dense side pays generate()
  per-step machinery the lean spec loop doesn't, and spec's timed windows
  exclude cache.crop — so treat even these as upper bounds. Definitive
  warm-vs-warm lean-loop rerun (sdtiming mode, --warm-baseline, all 20
  prompts timed) in flight; quote nothing from this bullet to outsiders
  except "~1.15×, measurement being finalized."
