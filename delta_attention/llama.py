...
from .delta_kernel import attention as qsa_kernel
...
class LlamaAttention(nn.Module):
    ...
    def hip_attention_interface(
        ...
        assert self.config._attn_implementation in ["hip_attention", "window"]
        ...
        if self.config._attn_implementation == "window":
            ...
            output = block_sparse_attention(
                q=(query * scaling).to(query.dtype),
                k=key,
                v=value,
            ...
            # hip landmarking requires keys and values to be multiples of 1024. position_ids will be used as 
            # a mask internally, so just pad them here.
            if key.size(1) % 1024 != 0:
                BSZ, TSRC, HEAD_KV, HID = key.shape
                to_pad = 1024 - (key.size(1) % 1024)
                key = torch.cat((key, torch.zeros(BSZ, to_pad, HEAD_KV, HID, device=key.device, dtype=key.dtype)), dim=1)
                value = torch.cat((value, torch.zeros(BSZ, to_pad, HEAD_KV, HID, device=key.device, dtype=key.dtype)), dim=1)
            ...
            output, _metadata = hip_attention(
                q=(query * args.sm_scale).to(query.dtype),
                k=key,
                v=value,
                args=args,
            )

            last_dense = self.config.hip_attention_last_dense
            idx = torch.arange(query.size(1) - last_dense, query.size(1), device=query.device)
            if last_dense > 0:
                q = query[:, idx].transpose(1, 2)
                k = key[:, :query.size(1)].transpose(1, 2)
                v = value[:, :query.size(1)].transpose(1, 2)

                k = repeat_kv(k, module.num_key_value_groups)
                v = repeat_kv(v, module.num_key_value_groups)
                dense_attn_output = qsa_kernel(q, k, v, idx.unsqueeze(0), args.sm_scale)
                dense_attn_output = dense_attn_output.transpose(1, 2)

                output[:, idx] = dense_attn_output
    ...
    def delta_forward(
        ...
        **kwargs,
        ...
        lambd = self.config.delta_lambda
        ...
        dense_attn_output = qsa_kernel(
            recomp_query_states, key_states, value_states, idx.unsqueeze(0), scaling
        )
        dense_attn_output = dense_attn_output.transpose(1, 2)

        if mode == "recompute":
            # we are only recomputing some tokens, just scatter outputs and return
        ...
        if self.config._attn_implementation != "eager":
            ...
            if self.config._attn_implementation == "sdpa_rectangle":
                ...
                if self.config._attn_implementation == "hip_attention" and self.layer_idx in self.config.hip_attention_dense_layers:
                    ...
                    attention_interface = self.delta_forward
                    kwargs.update(
                        {
