...
def test_unknown_keys_recorded_not_passed():
    ...
    # log_drift / decode_mode / stride_policy are not (yet) Config fields:
    assert "log_drift" in unknown and "decode_mode" in unknown
    assert "--log-drift" not in cmd and "--decode-mode" not in cmd
